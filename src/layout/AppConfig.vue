...
const { changeThemeSettings, setScale, layoutConfig, changeMarginRTl } = useLayout();
...
const changeToRtl = () => {
    if (!layoutConfig.rtl.value) {
        document.querySelector('body').setAttribute('dir', 'rtl');
        changeMarginRTl();
    } else {
        document.querySelector('body').removeAttribute('dir', 'rtl');
    }
};
</script>
...
<template dir="ltr">
    <div>
        <button class="layout-config-button p-link" :class="{ 'layout-config-button-rtl': layoutConfig.rtl.value }" type="button" @click="onConfigButtonClick()">
            <i class="pi pi-cog"></i>
        </button>

        <Sidebar v-model:visible="visible" :position="layoutConfig.rtl.value ? 'left' : 'right'" :transitionOptions="'.3s cubic-bezier(0, 0, 0.2, 1)'" class="layout-config-sidebar w-20rem">
            <h5>Scale</h5>
            ...
            <h5>RTL mode</h5>
            <InputSwitch v-model="layoutConfig.rtl.value" @click="changeToRtl"></InputSwitch>
